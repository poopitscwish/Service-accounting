...
class CustomAdapterRecord(private val context:Context, private val dataSet: MutableList<Record>, private val users:MutableList<User>) :
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.listRecord.setOnClickListener{
            ...
            dialogLayout.findViewById<Spinner>(R.id.FIO).adapter = SpinnerAdapterUser(context, users)
            dialogLayout.findViewById<Spinner>(R.id.FIO).onItemSelectedListener = this@CustomAdapterRecord
            ...
            dialogLayout.findViewById<EditText>(R.id.editThick).setText(dataSet[position].THICK.toString())
            dialogLayout.findViewById<EditText>(R.id.editProduct).setText(dataSet[position].PRODUCT)
            ...
            builder.setNegativeButton("cancel"){ _, i -> Toast.makeText(this.context,
                "Отмена!", Toast.LENGTH_SHORT).show() }
            builder.setPositiveButton("OK") { _, i ->
                val recordFIOID = positionSpinner
                val recordStart = dialogLayout.findViewById<TextView>(R.id.begin).text.toString()
                val recordEnd = dialogLayout.findViewById<TextView>(R.id.end).text.toString()
                val recordThick = dialogLayout.findViewById<EditText>(R.id.editThick).text.toString().toInt()
                val recordProduct = dialogLayout.findViewById<EditText>(R.id.editProduct).text.toString()
                val record = Record(
                    position,
                    recordFIOID,
                    recordStart,
